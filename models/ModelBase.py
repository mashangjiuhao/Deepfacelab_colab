...
class ModelBase(object):
    def __init__(self, is_training=False,
                ...
                while True:
                    ...
                    if len(saved_models_names) != 0:
                        if silent_start:
                            ...
                            inp = 0#io.input_str(f"", "0", show_default_value=False )#后面内容为输入一个模型的名字r,d,0.
                            model_idx = -1
                            try:
                        ...
                        self.model_name = io.input_str(f"没有发现模型. 输入一个名词新建模型", "new")
        ...
        if self.model_data_path.exists():
            io.log_info (f"加载名为 {self.model_name} 的模型...")
        ...
        if silent_start:
            ...
            self.device_config = nn.DeviceConfig.BestGPU()#nn.DeviceConfig.GPUIndexes( force_gpu_idxs or nn.ask_choose_device_idxs(suggest_best_multi_gpu=True)) \
                                #if not cpu_only else nn.DeviceConfig.CPU()#号部分内容为选择cpu或者GPU
    ...
    def update_sample_for_preview(self, choose_preview_history=False, force_new=False):
        if self.sample_for_preview is None or choose_preview_history or force_new:
            if choose_preview_history and io.is_support_windows():
                io.log_info ("选择预览图. [p] - next. [enter] - confirm.")
    ...
    def ask_override(self):
        return self.is_training and self.iter != 0 and io.input_in_time ("两秒内按下Enter键可以重新设置模型参数...", 5 if io.is_colab() else 2 )
    ...
    def ask_autobackup_hour(self, default_value=0):
        ...
        self.options['autobackup_hour'] = io.input_int(f"几个小时备份一次? Autobackup every N hour", default_autobackup_hour, add_info="0..24", help_message="Autobackup model files with preview every N hour. Latest backup located in model/<>_autobackups/01")
    ...
    def ask_write_preview_history(self, default_value=False):
        ...
        self.options['write_preview_history'] = io.input_bool(f"保存预览历史记录 Write preview history", default_write_preview_history, help_message="Preview history will be writed to <ModelName>_history folder.")

        if self.options['write_preview_history']:
            if io.is_support_windows():
                self.choose_preview_history = io.input_bool("选择预览图片 Choose image for the preview history:", False)
            elif io.is_colab():
                self.choose_preview_history = io.input_bool("随机选择预览图", False, help_message="Preview image history will stay stuck with old faces if you reuse the same model on different celebs. Choose no unless you are changing src/dst to a new person")

    def ask_target_iter(self, default_value=0):
        default_target_iter = self.load_or_def_option('target_iter', default_value)
        self.options['target_iter'] = max(0, io.input_int("目标迭代次数 Target iteration", default_target_iter))

    def ask_random_flip(self):
        default_random_flip = self.load_or_def_option('random_flip', True)
        self.options['random_flip'] = io.input_bool("人脸随机翻转 Flip faces randomly", default_random_flip, help_message="Predicted face will look more naturally without this option, but src faceset should cover all face directions as dst faceset.")
    ...
    def ask_batch_size(self, suggest_batch_size=None):
        ...
        self.options['batch_size'] = self.batch_size = max(0, io.input_int("批量大小 Batch_size", default_batch_size, help_message="Larger batch size is better for NN's generalization, but it can cause Out of Memory error. Tune this value for your videocard manually."))
    ...
    def get_loss_history_preview(loss_history, iter, w, c):
        ...
        lh_text = '迭代: %d  By deepfaker.xyz 托尼是塔克' % (iter) if iter != 0 else ''
